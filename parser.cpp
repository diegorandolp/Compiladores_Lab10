    ...
    if (match(Token::ID)) {
        ...
        if(match(Token::THEN)) {
            while (!check(Token::ELSE) and !check(Token::ENDIF) and !isAtEnd()) {
                if (!check(Token::PC)) {
                    cout << "then" << endl;
                    slis1.push_back(parseStatement());
                } else {
                    match(Token::PC);
                }
            }
            if (match(Token::ELSE)) {
                while (!check(Token::ENDIF) and !isAtEnd()) {
                    if (!check(Token::PC)) {
                        cout << "else" << endl;
                        slis2.push_back(parseStatement());
                    } else {
                        match(Token::PC);
                    }
                }
        ...
        s = new IfStatement(e, slis1, slis2);
...
Exp *Parser::parseCExp() {
    ...
    if (match(Token::MENOR) || match(Token::MENOREQ) || match(Token::EQUAL) ) {
        BinaryOp op;
        if (previous->type == Token::MENOR) {
            op = LESS_OP;
        } else if(previous->type == Token::EQUAL) {
            op = EQEQ_OP;
        } else if (previous->type == Token::MENOREQ) {
            op = LESSEQ_OP;
        }
